...
navigate_table(Dir, Lines, #table{id = TableId} = Table, Tree, Bounds) ->
    case resolved_table_visible_height(Tree, TableId, Table, Bounds) of
        undefined ->
            nit_nav:navigate_table(Dir, Lines, Table);
        VisibleHeight ->
            nit_nav:navigate_table(Dir, Lines, VisibleHeight, Table)
    end.
...
navigate_list(Dir, Lines, #list{id = ListId} = List, Tree, Bounds) ->
    case resolved_list_visible_height(Tree, ListId, List, Bounds) of
        undefined ->
            nit_nav:navigate_list(Dir, Lines, List);
        VisibleHeight ->
            nit_nav:navigate_list(Dir, Lines, VisibleHeight, List)
    end.
...
navigate_tree(Dir, TreeEl, Bounds) ->
    nit_tree_nav:navigate(Dir, TreeEl, Bounds).
...
navigate_tree(Dir, Lines, #tree{id = TreeId} = TreeEl, Tree, Bounds) ->
    ...
    nit_tree_nav:navigate(Dir, Lines, VisibleHeight, TreeEl).

scroll_tree(Dir, Lines, #tree{id = TreeId} = TreeEl, Tree, Bounds) ->
    VisibleHeight = resolved_tree_visible_height(Tree, TreeId, TreeEl, Bounds),
    nit_tree_nav:scroll(Dir, Lines, VisibleHeight, TreeEl).

toggle_tree_node(Dir, TreeEl, Bounds) ->
    nit_tree_nav:toggle(Dir, TreeEl, Bounds).
...
resolved_list_visible_height(Tree, ListId, #list{items = Items, height = H}, Bounds) ->
    case nit_bounds:find_element_bounds(Tree, ListId, Bounds) of
        {ok, #bounds{height = ResolvedHeight}} ->
...
resolved_tree_visible_height(Tree, TreeId, TreeEl, Bounds) ->
    case nit_bounds:find_element_bounds(Tree, TreeId, Bounds) of
        {ok, #bounds{height = ResolvedHeight}} ->
            max(1, ResolvedHeight);
        not_found ->
            max(1, nit_tree_nav:resolved_height(TreeEl, Bounds))
    end.

resolved_scroll_bounds(Tree, ScrollId, Bounds) ->
    case nit_bounds:find_element_bounds(Tree, ScrollId, Bounds) of
...
collect_scroll_ids(#scroll{id = Id} = El) ->
    ...
    Current ++ lists:flatmap(fun collect_scroll_ids/1, nit_element:children(El));
collect_scroll_ids(El) ->
    lists:flatmap(fun collect_scroll_ids/1, nit_element:children(El)).
...
focus_container_for(Tree, ElementId) ->
    case nit_focus:find_container(Tree, ElementId) of
        undefined -> ElementId;
        Container -> Container
    end.
...
navigate_element(Dir, ElementId, Tree, Bounds, Cb, US) ->
    case nit_focus:find_element(Tree, ElementId) of
        ...
        #tree{} = TreeEl when Dir =:= up; Dir =:= down ->
            ...
            NewTree = nit_tree:update(Tree, ElementId, NewTreeEl),
        ...
        #list{} = List when Dir =:= up; Dir =:= down ->
            ...
            NewTree = nit_tree:update(Tree, ElementId, NewList),
        ...
        #scroll{} = Scroll when Dir =:= up; Dir =:= down ->
            ...
            NewTree = nit_tree:update(Tree, ElementId, NewScroll),
            {ok, NewTree, US};
        _ ->
            unhandled
    end.
...
page_navigate_element(Dir, ElementId, Tree, Bounds, Cb, US) ->
    case nit_focus:find_element(Tree, ElementId) of
        ...
        #tree{} = TreeEl ->
            ...
            NewTreeEl = nit_tree_nav:navigate(Dir, Lines, Lines, TreeEl),
            NewTree = nit_tree:update(Tree, ElementId, NewTreeEl),
            NewUS = maybe_tree_selection_user_state(Cb, TreeEl, NewTreeEl, US),
            {ok, NewTree, NewUS};
        #scroll{} = Scroll ->
        ...
        {ok, Element} ->
            ...
            case page_navigate_element_raw(Dir, Element, ElementId, Tree, Bounds, Cb, US) of
                {ok, NewElement, NewTree0, NewUS} ->
                    ...
                    NewTree = nit_tree:update(NewTree0, ContainerId, NewTabs),
                    {ok, NewTree, NewUS};
                unhandled ->
                    unhandled
            end;
...
apply_char_input(Tree, FocusedChild, Char) ->
    case nit_focus:find_element(Tree, FocusedChild) of
        #input{id = Id} = Input ->
            ...
            NewTree = nit_tree:update(Tree, FocusedChild, NewInput),
...
move_input_cursor(Tree, InputId, Dir, Select) ->
    case nit_focus:find_element(Tree, InputId) of
        #input{} = Input ->
            ...
            {ok, nit_tree:update(Tree, InputId, NewInput)};
        _ ->
            false
    end.
...
cycle_focus(Dir, ContainerIds, CurrentContainer, Tree) ->
    ...
    ChildIds = nit_focus:collect_children(Tree, NewContainer),
...
init_focus_state(CallbackModule, InitArg) ->
    ...
    ChildIds = nit_focus:collect_children(Tree, FocusedContainer),
