...
%% #nit_state but scoped to the modal subtree.
-record(modal_focus, {
    container :: term(),
    child :: term(),
    container_ids :: [term()]
...
-record(nit_state, {
    ...
    prev_screen :: undefined | nit_screen:screen(),  %% Previous screen for diff rendering
...
init({_Name, CallbackModule, InitArg}) ->
    %% Note: nit_tty and nit_input are started by nitui application,
    ...
    nit_tty:set_resize_target(self()),
    %% Get terminal size
    Bounds = case nit_tty:get_size() of
    ...
    {UserState, Tree, ContainerIds, FocusedContainer, FocusedChild} =
        nit_engine:init_focus_state(CallbackModule, InitArg),
...
handle_call(get_state, _From, State = #nit_state{user_state = UserState}) ->
...
handle_info(cursor_blink, State = #nit_state{cursor_visible = Visible, cursor_timer = _Timer}) ->
    ...
    NewState = State#nit_state{cursor_visible = NewVisible, cursor_timer = NewTimer},
    FinalState = render_diff(NewState),
    {noreply, FinalState};

%% Handle periodic refresh timer for live updates
handle_info(refresh_tick, State = #nit_state{callback = Cb, user_state = US, tree = OldTree}) ->
    ...
    {noreply, FinalState#nit_state{refresh_timer = NewTimer}};
...
terminate(_Reason, #nit_state{cursor_timer = CursorTimer, refresh_timer = RefreshTimer}) ->
...
%% #nit_state tracks the modal's own {container, child, container_ids}
...
init_modal_focus(Modal) ->
    ContainerIds = nit_focus:collect_containers(Modal),
    Container = case ContainerIds of [First | _] -> First; [] -> undefined end,
    ChildIds = nit_focus:collect_children(Modal, Container),
    Child = case ChildIds of
        ...
        [] ->
            ...
            case nit_focus:collect_children(#box{children = modal_children(Modal)}, undefined) of
                [F | _] -> F;
                [] -> undefined
            end
    end,
...
active_focus(#nit_state{modal = undefined, tree = Tree,
                        focused_container = FC, focused_child = Ch,
                        container_ids = Ids}) ->
    {Tree, FC, Ch, Ids, main};
active_focus(#nit_state{modal = Modal, modal_focus = undefined} = State) ->
    active_focus(State#nit_state{modal_focus = init_modal_focus(Modal)});
active_focus(#nit_state{modal = Modal,
...
set_active(main, State, Tree, FC, Ch, Ids) ->
    State#nit_state{tree = Tree,
                    focused_container = FC,
                    focused_child = Ch,
                    container_ids = Ids};
set_active(modal, State, Modal, FC, Ch, Ids) ->
    State#nit_state{modal = Modal,
                    modal_focus = #modal_focus{container = FC, child = Ch,
                                               container_ids = Ids}}.
...
set_active_child(main, State, Ch) ->
    State#nit_state{focused_child = Ch};
set_active_child(modal, State = #nit_state{modal_focus = MF}, Ch) ->
    State#nit_state{modal_focus = MF#modal_focus{child = Ch}}.
...
handle_input(escape, State = #nit_state{fullscreen = #fullscreen{}, user_state = US}) ->
    exit_fullscreen(US, State, undefined);
handle_input({ctrl, $c}, State = #nit_state{callback = Cb, user_state = US}) ->
...
handle_input({key, page_up}, State = #nit_state{modal = Modal}) when Modal =/= undefined ->
    {noreply, State};
handle_input({key, page_down}, State = #nit_state{modal = Modal}) when Modal =/= undefined ->
    {noreply, State};
...
handle_input(delete, State) ->
    handle_delete(State);
handle_input({mouse, click, left, Col, Row}, State = #nit_state{modal = Modal, bounds = Bounds}) when Modal =/= undefined ->
...
handle_input({mouse, _, _, _, _}, State) ->
    {noreply, State};
handle_input(_Event, State = #nit_state{modal = Modal}) when Modal =/= undefined ->
    {noreply, State};
handle_input(Event, State) ->
    forward_event(Event, State).
...
handle_focus_next(State) ->
    ...
    {NewContainer, NewChild} = nit_engine:cycle_focus(next, Ids, Current, Tree),
...
handle_focus_prev(State) ->
    {Tree, Current, _Ch, Ids, Where} = active_focus(State),
    {NewContainer, NewChild} = nit_engine:cycle_focus(prev, Ids, Current, Tree),
    NewState0 = set_active(Where, State, Tree, NewContainer, NewChild, Ids),
    NewState = update_cursor_timer(NewState0),
    FinalState = render_diff(NewState),
    {noreply, FinalState}.
...
handle_modal_click(Col, Row, Modal, Bounds, State) ->
    case nit_hit:find_at(Modal, Col, Row, Bounds) of
        {button, ButtonId} ->
        ...
        #button{id = Id, on_click = Handler} ->
            case call_handler(Cb, {click, Id, Handler}, US) of
                ...
                {modal, NewModal, NewUS} ->
                    NewState = activate_modal(State#nit_state{user_state = NewUS}, NewModal),
                    FinalState = render_diff(NewState),
                    {noreply, FinalState};
                Other ->
    ...
    case nit_engine:activation_target(Tree, Container, FocusedChild) of
        ...
        #input{id = Id, value = Value, on_submit = Handler} ->
            case call_handler(Cb, {submit, Id, Value, Handler}, US) of
                ...
                {push, NewModule, Args, NewUS} -> do_push(NewModule, Args, State#nit_state{user_state = NewUS});
        ...
        #list{id = Id, items = Items, selected = SelIdx} ->
            Event = {list_select, Id, SelIdx, nit_engine:list_selected_item(Items, SelIdx)},
...
handle_activate_in_tabs(Container, Tree, Cb, US, State) ->
    case nit_focus:find_element(Tree, Container) of
        #tabs{tabs = TabList, active_tab = ActiveTab0} ->
            ActiveTab = nit_engine:resolve_active_tab(ActiveTab0, [T#tab.id || T <- TabList]),
            case lists:keyfind(ActiveTab, #tab.id, TabList) of
                #tab{content = Content} ->
                    case nit_engine:find_table_in_content(Content) of
...
handle_arrow(Dir, State = #nit_state{bounds = Bounds, callback = Cb, user_state = US}) ->
    case handle_input_cursor(Dir, false, State) of
        unhandled ->
...
handle_arrow_non_input(Dir, State, undefined, Child, Tree, _Bounds, _Cb, _US, modal) ->
    ...
    FlatIds = nit_focus:collect_children(#box{children = modal_children(Tree)}, undefined),
    case {Dir, FlatIds} of
        {_, []} -> {noreply, State};
        {up, _} ->
            NewChild = nit_focus:prev_focus(FlatIds, Child),
            navigate_modal_to_child(State, NewChild);
        {down, _} ->
            NewChild = nit_focus:next_focus(FlatIds, Child),
            navigate_modal_to_child(State, NewChild);
        {left, _} ->
            NewChild = nit_focus:prev_focus(FlatIds, Child),
            navigate_modal_to_child(State, NewChild);
...
handle_arrow_non_input(Dir, State, Container, Child, Tree, Bounds, Cb, US, _Where) ->
    ...
    ChildIds = nit_focus:collect_children(Tree, Container),
    case nit_focus:find_element(Tree, Container) of
        #tabs{id = Id, tabs = TabList, active_tab = ActiveTab} = Tabs ->
            ...
            case Dir of
                left ->
                    NewTabs = nit_engine:navigate_tabs(left, Tabs),
                    ...
                    NewState = State#nit_state{tree = NewTree, user_state = NewUS, focused_child = NewActiveTab},
                    FinalState = render_diff(NewState),
                    {noreply, FinalState};
                right ->
                    NewTabs = nit_engine:navigate_tabs(right, Tabs),
                    NewActiveTab = NewTabs#tabs.active_tab,
                    NewTree = nit_tree:update(Tree, Container, NewTabs),
                    NewUS = case call_handler(Cb, {tab_change, Id, NewActiveTab}, US) of
                        {noreply, S} -> S;
        ...
        #box{} ->
            ...
            case nit_engine:navigate_element(Dir, Child, Tree, Bounds, Cb, US) of
                {ok, NewTree, NewUS} ->
                    case _Where of
                        main ->
                        ...
                        modal ->
                            NewState = set_active_tree(modal,
                                                       State#nit_state{user_state = NewUS},
                                                       NewTree),
                ...
                unhandled ->
                    case Dir of
                        up ->
                            NewChild = nit_focus:prev_focus(ChildIds, Child),
                            NewState0 = set_active_child(_Where, State, NewChild),
                            NewState = update_cursor_timer(NewState0),
                            FinalState = render_diff(NewState),
                            {noreply, FinalState};
                        down ->
                            NewChild = nit_focus:next_focus(ChildIds, Child),
                            NewState0 = set_active_child(_Where, State, NewChild),
                            NewState = update_cursor_timer(NewState0),
                            FinalState = render_diff(NewState),
                            {noreply, FinalState};
        ...
        _ ->
            ...
            case nit_engine:navigate_element(Dir, Container, Tree, Bounds, Cb, US) of
                {ok, NewTree, NewUS} ->
                    case _Where of
                        main ->
                            apply_view_update(NewUS, State#nit_state{tree = NewTree}, NewTree);
...
handle_page(Dir, State = #nit_state{focused_container = Container, focused_child = Child,
    ...
    case nit_focus:find_element(Tree, Container) of
        #tabs{tabs = TabList, active_tab = ActiveTab0} = Tabs ->
            ...
            case lists:keyfind(ActiveTab, #tab.id, TabList) of
                #tab{content = Content} ->
                    case nit_engine:page_navigate_tab_content(Dir, Container, Tabs, Content, Tree, Bounds, Cb, US) of
                        {ok, NewTree, NewUS} ->
                            apply_view_update(NewUS, State#nit_state{tree = NewTree}, NewTree);
                        unhandled ->
                            unhandled
        ...
        #box{} ->
            case nit_engine:page_navigate_element(Dir, Child, Tree, Bounds, Cb, US) of
                {ok, NewTree, NewUS} ->
                    apply_view_update(NewUS, State#nit_state{tree = NewTree}, NewTree);
                unhandled ->
                    unhandled
        ...
        _ ->
            case nit_engine:page_navigate_element(Dir, Container, Tree, Bounds, Cb, US) of
                {ok, NewTree, NewUS} ->
                    apply_view_update(NewUS, State#nit_state{tree = NewTree}, NewTree);
                unhandled ->
                    unhandled
...
handle_backspace(State = #nit_state{callback = Cb, user_state = US}) ->
    {Tree, _FC, _Ch, _Ids, Where} = active_focus(State),
    InputId = focused_input_id(State),
    case nit_engine:apply_backspace(Tree, InputId) of
        {ok, NewTree, Id, NewValue} ->
            NewUS = case call_handler(Cb, {input, Id, NewValue}, US) of
                {noreply, S} -> S;
                _ -> US
...
handle_delete(State = #nit_state{callback = Cb, user_state = US}) ->
    {Tree, _FC, _Ch, _Ids, Where} = active_focus(State),
    InputId = focused_input_id(State),
    case nit_engine:apply_delete_input(Tree, InputId) of
        {ok, NewTree, Id, NewValue} ->
            NewUS = case call_handler(Cb, {input, Id, NewValue}, US) of
                {noreply, S} -> S;
                _ -> US
...
handle_input_cursor(Dir, Select, State) ->
    ...
    case focused_input_id(State) of
        undefined ->
            unhandled;
        InputId ->
            case nit_engine:move_input_cursor(Tree, InputId, Dir, Select) of
                {ok, NewTree} ->
...
handle_input_select_all(State) ->
    ...
    case focused_input_id(State) of
        undefined ->
            unhandled;
        InputId ->
            case nit_engine:select_all_input(Tree, InputId) of
                {ok, NewTree} ->
                    render_input_state(Where, NewTree, State);
                false ->
                    unhandled
...
handle_input_drag(_Col, _Row, State = #nit_state{modal = Modal}) when Modal =/= undefined ->
    {noreply, State};
handle_input_drag(Col, _Row, State = #nit_state{tree = Tree, bounds = Bounds}) ->
    case focused_input_id(State) of
        undefined ->
            {noreply, State};
        InputId ->
            case nit_focus:find_element(Tree, InputId) of
                #input{} = Input ->
                    case nit_bounds:find_element_bounds(Tree, InputId, Bounds) of
                        {ok, InputBounds} ->
                            Pos = input_position_from_col(Input, InputBounds, Col),
                            case nit_engine:position_input_cursor(Tree, InputId, Pos, true) of
...
focused_input_id(State) ->
    ...
    case nit_focus:find_element(Tree, FocusedChild) of
        #input{} ->
            FocusedChild;
        _ ->
            case nit_focus:find_element(Tree, FocusedContainer) of
...
handle_scroll(Dir, Lines, Col, Row, State = #nit_state{tree = Tree, bounds = Bounds}) ->
    case nit_hit:find_at(Tree, Col, Row, Bounds) of
        ...
        {tree, TreeId} ->
            scroll_tree(Dir, Lines, TreeId, State);
        _ ->
            case nit_engine:find_scroll_at(Tree, Col, Row, Bounds) of
                {ok, ScrollId} ->
...
scroll_list(Dir, Lines, ListId, State = #nit_state{tree = Tree, bounds = Bounds,
                                                   callback = Cb, user_state = US}) ->
    case nit_focus:find_element(Tree, ListId) of
        #list{} = List ->
            NewList = nit_engine:navigate_list(Dir, Lines, List, Tree, Bounds),
            NewTree = nit_tree:update(Tree, ListId, NewList),
            Event = {list_select, ListId, NewList#list.selected,
                     nit_engine:list_selected_item(NewList#list.items, NewList#list.selected)},
...
scroll_table(Dir, Lines, TableId, State = #nit_state{tree = Tree, bounds = Bounds,
                                                     callback = Cb, user_state = US}) ->
    case nit_focus:find_element(Tree, TableId) of
        #table{} = Table ->
            NewTable = nit_engine:navigate_table(Dir, Lines, Table, Tree, Bounds),
...
scroll_tree(Dir, Lines, TreeId, State = #nit_state{tree = Tree, bounds = Bounds,
                                                   user_state = US}) ->
    case nit_focus:find_element(Tree, TreeId) of
        #tree{} = TreeEl ->
            NewTreeEl = nit_engine:scroll_tree(Dir, Lines, TreeEl, Tree, Bounds),
            NewTree = nit_tree:update(Tree, TreeId, NewTreeEl),
            apply_view_update(US, State#nit_state{tree = NewTree}, NewTree);
        _ ->
            {noreply, State}
    end.

scroll_container(Dir, Lines, ScrollId, State = #nit_state{tree = Tree, bounds = Bounds}) ->
    case nit_focus:find_element(Tree, ScrollId) of
        #scroll{} = Scroll ->
            NewScroll = nit_engine:navigate_scroll(Dir, Lines, Scroll, Tree, Bounds),
            NewTree = nit_tree:update(Tree, ScrollId, NewScroll),
            NewState = State#nit_state{tree = NewTree},
            FinalState = render_diff(NewState),
            {noreply, FinalState};
        _ ->
...
scroll_focused(Dir, Lines, State = #nit_state{tree = Tree,
                                              focused_child = FocusedChild,
                                              focused_container = FocusedContainer}) ->
    case nit_engine:scroll_target(Tree, FocusedChild, FocusedContainer) of
        {list, Id} ->
...
handle_mouse_click(Col, Row, State = #nit_state{tree = Tree, bounds = Bounds,
                                                callback = Cb, user_state = US}) ->
    case nit_hit:find_at(Tree, Col, Row, Bounds) of
        {tab, TabsId, TabId} ->
            case nit_focus:find_element(Tree, TabsId) of
                #tabs{} = Tabs ->
                    NewTabs = Tabs#tabs{active_tab = TabId},
                    NewTree = nit_tree:update(Tree, TabsId, NewTabs),
        ...
        {tabs_container, TabsId} ->
            ...
            ChildIds = nit_focus:collect_children(Tree, TabsId),
            FirstChild = case ChildIds of [C|_] -> C; [] -> undefined end,
            NewState = State#nit_state{focused_container = TabsId, focused_child = FirstChild},
            FinalState = render_diff(NewState),
            {noreply, FinalState};
        {box, BoxId} ->
            %% Clicked on box container (border or empty space)
            ChildIds = nit_focus:collect_children(Tree, BoxId),
            FirstChild = case ChildIds of [C|_] -> C; [] -> undefined end,
            ...
            case nit_focus:find_element(Tree, InputId) of
                #input{} = Input ->
                    case nit_bounds:find_element_bounds(Tree, InputId, Bounds) of
                        {ok, InputBounds} ->
                            Pos = input_position_from_col(Input, InputBounds, Col),
                            case nit_engine:start_input_selection(Tree, InputId, Pos) of
                                {ok, NewTree} ->
                                    NewState0 = State#nit_state{
                                        tree = NewTree,
        ...
        {table_header, TableId, ColumnId} ->
            case nit_focus:find_element(Tree, TableId) of
                #table{} = Table ->
                    ...
                    SortedTable = nit_el_table:toggle_sort(Table, ColumnId),
                    case SortedTable =:= Table of
                        false ->
                            NewTree = nit_tree:update(Tree, TableId, SortedTable),
                        ...
                        true ->
                            ...
                            case call_handler_with_debug(Cb, Event, US, FocusedState) of
                                ...
                                {handled, {modal, Modal, NewUS}, _} ->
                                    NewState = activate_modal(FocusedState#nit_state{user_state = NewUS}, Modal),
                                    FinalState = render_diff(NewState),
                                    {noreply, FinalState};
                                {handled, {switch, NewModule, Args}, _} ->
                                ...
                                {handled, {push, NewModule, Args, NewUS}, _} ->
                                    do_push(NewModule, Args, FocusedState#nit_state{user_state = NewUS});
                                {handled, pop, _} ->
                                    do_pop(FocusedState);
                                {handled, {stop, Reason, _NewUS}, _} ->
        ...
        {table, TableId} ->
            ...
            Container = nit_engine:focus_container_for(Tree, TableId),
            NewState = State#nit_state{focused_container = Container, focused_child = TableId},
            FinalState = render_diff(NewState),
            {noreply, FinalState};
        {list_item, ListId, ItemIdx} ->
            ...
            case nit_focus:find_element(Tree, ListId) of
                #list{} = List ->
                    ...
                    NewTree = nit_tree:update(Tree, ListId, NewList),
                    Container = nit_engine:focus_container_for(Tree, ListId),
                    Event = {list_select, ListId, ItemIdx,
                             nit_engine:list_selected_item(NewList#list.items, ItemIdx)},
                    NewUS = nit_engine:selection_user_state(Cb, Event, US),
                    apply_view_update(NewUS, State#nit_state{tree = NewTree,
                                               focused_container = Container,
        ...
        {list, ListId} ->
            ...
            Container = nit_engine:focus_container_for(Tree, ListId),
            NewState = State#nit_state{focused_container = Container, focused_child = ListId},
            FinalState = render_diff(NewState),
            {noreply, FinalState};
        {tree_toggle, TreeId, NodeId} ->
            case nit_focus:find_element(Tree, TreeId) of
                #tree{} = TreeEl ->
                    Container = nit_engine:focus_container_for(Tree, TreeId),
                    FocusedState = State#nit_state{focused_container = Container,
                                                  focused_child = TreeId},
                    SelectedTreeEl = nit_tree_nav:select(NodeId, TreeEl, Bounds),
                    NewTreeEl = nit_tree_nav:toggle_selected(SelectedTreeEl, Bounds),
                    NewTree = nit_tree:update(Tree, TreeId, NewTreeEl),
                    NewUS = nit_engine:maybe_tree_selection_user_state(Cb, TreeEl, NewTreeEl, US),
                    apply_view_update(NewUS, FocusedState#nit_state{tree = NewTree,
        ...
        {tree_node, TreeId, NodeId} ->
            case nit_focus:find_element(Tree, TreeId) of
                #tree{} = TreeEl ->
                    ...
                    NewTreeEl = nit_tree_nav:select(NodeId, TreeEl, Bounds),
                    NewTree = nit_tree:update(Tree, TreeId, NewTreeEl),
                    NewUS = nit_engine:maybe_tree_selection_user_state(Cb, TreeEl, NewTreeEl, US),
                    NewState = FocusedState#nit_state{tree = NewTree, user_state = NewUS},
                    case WasSelected of
                        true ->
        ...
        {tree, TreeId} ->
            Container = nit_engine:focus_container_for(Tree, TreeId),
            NewState = State#nit_state{focused_container = Container, focused_child = TreeId},
            FinalState = render_diff(NewState),
            {noreply, FinalState};
        {status_bar_item, Key} ->
...
do_update(UpdateFun, State = #nit_state{user_state = US}) ->
    NewUS = UpdateFun(US),
...
rebuild_view_state(NewUS, State = #nit_state{fullscreen = undefined}, MergeFromTree) ->
    BaseTree = rebuild_base_tree(NewUS, State, MergeFromTree),
    {Container, Child, ContainerIds} =
        resolve_focus(BaseTree, State#nit_state.focused_container,
                      State#nit_state.focused_child),
    State#nit_state{
        user_state = NewUS,
        tree = BaseTree,
        focused_container = Container,
...
rebuild_view_state(NewUS, State = #nit_state{fullscreen = FS0}, MergeFromTree) ->
    BaseTree = rebuild_base_tree(NewUS, State, MergeFromTree),
    case fullscreen_tree(BaseTree, FS0#fullscreen.id) of
        {ok, FullTree} ->
            {Container, Child, ContainerIds} =
                resolve_focus(FullTree, State#nit_state.focused_container,
                              State#nit_state.focused_child),
            State#nit_state{
                user_state = NewUS,
                tree = FullTree,
...
merge_source_tree(#nit_state{fullscreen = #fullscreen{id = Id, tree = BaseTree},
    ...
    nit_tree:update(BaseTree, Id, FullTree);
merge_source_tree(#nit_state{fullscreen = #fullscreen{id = Id, tree = BaseTree}}, MergeFromTree) ->
    nit_tree:update(BaseTree, Id, MergeFromTree).

resolve_focus(Tree, CurrentContainer, CurrentChild) ->
    ContainerIds = nit_focus:collect_containers(Tree),
...
fullscreen_saved_state(Id, BaseTree, #nit_state{fullscreen = undefined} = State) ->
    #fullscreen{
        id = Id,
        tree = BaseTree,
        focused_container = State#nit_state.focused_container,
        focused_child = State#nit_state.focused_child,
        container_ids = State#nit_state.container_ids
    };
fullscreen_saved_state(Id, BaseTree, #nit_state{fullscreen = FS}) ->
    FS#fullscreen{id = Id, tree = BaseTree}.

toggle_fullscreen(Id, NewUS, State = #nit_state{fullscreen = #fullscreen{id = Id}},
                  MergeFromTree) ->
...
fullscreen_tree(Tree, Id) ->
    case nit_focus:find_element(Tree, Id) of
        undefined -> not_found;
...
do_switch(NewModule, Args, State) ->
    try nit_engine:init_focus_state(NewModule, Args) of
...
do_push(NewModule, Args, State = #nit_state{
    ...
    try NewModule:init(Args) of
        {ok, NewUS} ->
            NewTree = nit_engine:call_view(NewModule, NewUS, undefined),
            ContainerIds = nit_focus:collect_containers(NewTree),
            NewContainer = case ContainerIds of [C|_] -> C; [] -> undefined end,
            ChildIds = nit_focus:collect_children(NewTree, NewContainer),
...
do_pop(State = #nit_state{nav_stack = []}) ->
    %% Nothing to pop - stay on current view
    {noreply, State};
do_pop(State = #nit_state{nav_stack = [Entry | Rest]}) ->
    #nav_entry{
        callback = Cb,
        user_state = US,
        tree = OldTree,
    ...
    NewTree = nit_tree:merge_state(OldTree, nit_engine:call_view(Cb, US, OldTree)),
    ...
    ChildIds = nit_focus:collect_children(NewTree, NewFC),
...
forward_event(Event, State = #nit_state{callback = Cb, user_state = US}, MergeFromTree) ->
    %% Store event in debug_event for display
    StateWithDebug = State#nit_state{debug_event = Event},
    ...
    case erlang:function_exported(Cb, handle_event, 2) of
        true ->
            case Cb:handle_event(Event, US) of
                {unhandled, NewUS} ->
                    {unhandled, NewUS, State#nit_state{debug_event = Event}};
                Other ->
                    {handled, Other, State}
            end;
        false ->
            {unhandled, US, State#nit_state{debug_event = Event}}
    end.
...
render_diff(State = #nit_state{tree = Tree, bounds = Bounds,
    ...
    AnsiOutput = case Modal of
        undefined ->
            nit_render:render_two_level(Tree, Bounds, Container, Child, RenderOpts);
        _ ->
    ...
    case nit_unicode:contains_wide(AnsiOutput) of
        ...
        false ->
            ...
            NewScreen = nit_screen:from_ansi(AnsiOutput, W, H),
            %% Diff against previous screen and output only changes
            case PrevScreen of
                undefined ->
                    %% First render - output everything
                    nit_tty:write(nit_screen:to_ansi(NewScreen));
                _ ->
                    case nit_screen:get_size(PrevScreen) of
                        {W, H} ->
                            %% Same size - diff render
                            DiffOutput = nit_screen:diff(PrevScreen, NewScreen),
                            case DiffOutput of
                                [] ->
                                    %% No changes - nothing to output
                                    ok;
                                _ ->
                                    nit_tty:write(DiffOutput)
                            end;
                        _ ->
            ...
            State#nit_state{prev_screen = NewScreen, mounted_ids = NewMounted}
    end.
...
update_cursor_timer(State = #nit_state{cursor_timer = OldTimer}) ->
    ...
    FocusedElement = nit_focus:find_element(Tree, FocusedChild),
    ...
    case {IsInput, OldTimer} of
        {true, undefined} ->
            ...
            State#nit_state{cursor_visible = true, cursor_timer = Timer};
        {false, undefined} ->
            %% Not an input, no timer - nothing to do
            State;
        {true, _} ->
            %% Focusing on input, timer already running - reset visibility
            State#nit_state{cursor_visible = true};
        {false, _} ->
            %% Not an input but timer running - stop it
            erlang:cancel_timer(OldTimer),
            State#nit_state{cursor_visible = true, cursor_timer = undefined}
    end.
