...
render(#box{border = Border, title = Title, children = Children,
    ...
    [
        nit_ansi:render_box_border(ActualX, ActualY, Width, Height, BorderStyle, Title, Border),
        render_children(Children, ChildBounds, Opts)
    ].
...
height(#box{height = H, border = Border}, Bounds) ->
    BaseHeight = nit_ansi:resolve_size(H, Bounds#bounds.height),
    case Border of
        none -> BaseHeight;
...
width(#box{width = W}, Bounds) ->
    nit_ansi:resolve_size(W, Bounds#bounds.width).
...
render_children(Children, Bounds, Opts) ->
    [nit_element:render(Child, Bounds, Opts) || Child <- Children].
