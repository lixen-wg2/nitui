...
navigate_without_selection_selects_first_node_test() ->
    Tree = simple_tree(),
    Navigated = nit_tree_nav:navigate(down, Tree, test_bounds()),
...
navigate_down_scrolls_selected_node_into_view_test() ->
    Tree = simple_tree(),
    Navigated = nit_tree_nav:navigate(down, Tree#tree{selected = node2}, test_bounds()),
...
toggle_left_on_child_selects_parent_test() ->
    ...
    Toggled = nit_tree_nav:toggle(left, Tree, test_bounds()),
...
toggle_right_on_expanded_parent_selects_first_child_test() ->
    ...
    Toggled = nit_tree_nav:toggle(right, Tree, test_bounds()),
...
toggle_selected_expands_collapsed_node_test() ->
    ...
    Toggled = nit_tree_nav:toggle_selected(Tree, test_bounds()),
...
row_node_uses_offset_test() ->
    ...
    ?assertEqual({ok, node2}, nit_tree_nav:row_node(Tree, test_bounds(), 1)),
    ?assertEqual({ok, node3}, nit_tree_nav:row_node(Tree, test_bounds(), 2)).
...
visible_nodes_respects_large_offset_test() ->
    ...
    Visible = nit_tree_nav:visible_nodes(Tree, test_bounds()),
...
scroll_clamps_offset_without_changing_selection_test() ->
    ...
    Scrolled = nit_tree_nav:scroll(down, 10, 2, Tree),
    ?assertEqual(node1, Scrolled#tree.selected),
    ?assertEqual(1, Scrolled#tree.offset).
