%%%-------------------------------------------------------------------
%%% @doc Unit tests for nit_hit module.
%%%
...
button_hit_test() ->
    ...
    ?assertEqual({button, my_btn}, nit_hit:find_at(Tree, 8, 3, Bounds)).

button_miss_above_test() ->
    Tree = #button{id = my_btn, x = 5, y = 2, label = <<"Click">>},
    Bounds = default_bounds(),
    ?assertEqual(not_found, nit_hit:find_at(Tree, 8, 2, Bounds)).

button_miss_left_test() ->
    Tree = #button{id = my_btn, x = 5, y = 2, label = <<"Click">>},
    Bounds = default_bounds(),
    ?assertEqual(not_found, nit_hit:find_at(Tree, 5, 3, Bounds)).

button_miss_right_test() ->
    Tree = #button{id = my_btn, x = 5, y = 2, width = 10, label = <<"Click">>},
    Bounds = default_bounds(),
    ?assertEqual(not_found, nit_hit:find_at(Tree, 20, 3, Bounds)).
...
input_hit_test() ->
    ...
    ?assertEqual({input, my_input}, nit_hit:find_at(Tree, 15, 6, Bounds)).

input_miss_test() ->
    Tree = #input{id = my_input, x = 10, y = 5, width = 20},
    Bounds = default_bounds(),
    ?assertEqual(not_found, nit_hit:find_at(Tree, 5, 6, Bounds)).
...
table_hit_row_test() ->
    ...
    ?assertEqual({table_row, my_table, 1}, nit_hit:find_at(Tree, 5, 4, Bounds)).
...
table_hit_second_row_test() ->
    Tree = #table{id = my_table, x = 0, y = 0, width = 40, height = 10,
                  ...
                  rows = [[<<"Row 1">>], [<<"Row 2">>], [<<"Row 3">>]]},
    Bounds = default_bounds(),
    ?assertEqual({table_row, my_table, 2}, nit_hit:find_at(Tree, 5, 5, Bounds)).
...
table_hit_header_test() ->
    Tree = #table{id = my_table, x = 0, y = 0, width = 40, height = 10,
                  ...
                  rows = [[<<"<0.1.0>">>, <<"45 KB">>]]},
    Bounds = default_bounds(),
    ?assertEqual({table_header, my_table, pid}, nit_hit:find_at(Tree, 5, 2, Bounds)),
    ?assertEqual({table_header, my_table, mem}, nit_hit:find_at(Tree, 16, 2, Bounds)).
...
table_miss_test() ->
    Tree = #table{id = my_table, x = 5, y = 5, width = 20, height = 5,
                  columns = [], rows = []},
    Bounds = default_bounds(),
    ?assertEqual(not_found, nit_hit:find_at(Tree, 1, 1, Bounds)).
...
virtual_table_hit_row_test() ->
    Tree = #table{id = virtual_table, x = 0, y = 0, width = 40, height = 10,
                  ...
                  columns = [#table_col{header = <<"Col">>}]},
    Bounds = default_bounds(),
    ?assertEqual({table_row, virtual_table, 1}, nit_hit:find_at(Tree, 5, 4, Bounds)).
...
tree_hit_node_test() ->
    ...
    Bounds = default_bounds(),
    ?assertEqual({tree_toggle, my_tree, root}, nit_hit:find_at(Tree, 2, 1, Bounds)),
    ?assertEqual({tree_node, my_tree, root}, nit_hit:find_at(Tree, 5, 1, Bounds)),
    ?assertEqual({tree_node, my_tree, child}, nit_hit:find_at(Tree, 3, 2, Bounds)).
...
box_hit_child_test() ->
    ...
    %% nit_el_box). Button at x=2,y=2 with label "Inner" renders at row 3,
    %% cols 3-11. Should find the child button, not the box.
    ?assertEqual({button, inner_btn}, nit_hit:find_at(Tree, 5, 3, Bounds)).
...
box_hit_empty_space_test() ->
    ...
    ?assertEqual({box, my_box}, nit_hit:find_at(Tree, 5, 5, Bounds)).
...
box_non_focusable_miss_test() ->
    ...
    ?assertEqual(not_found, nit_hit:find_at(Tree, 5, 5, Bounds)).
...
nested_in_vbox_test() ->
    ...
    ?assertEqual({button, btn1}, nit_hit:find_at(Tree, 3, 1, Bounds)).
...
nested_in_panel_test() ->
    ...
    Bounds = default_bounds(),
    ?assertEqual({button, panel_btn}, nit_hit:find_at(Tree, 8, 6, Bounds)).
...
hbox_auto_children_share_width_hit_test() ->
    ...
    ?assertEqual({box, right_box}, nit_hit:find_at(Tree, 15, 2, Bounds)).
...
status_bar_item_hit_test() ->
    ...
    Bounds = default_bounds(),
    ?assertEqual({status_bar_item, <<"H">>}, nit_hit:find_at(Tree, 2, 1, Bounds)),
    ?assertEqual({status_bar_item, <<"H">>}, nit_hit:find_at(Tree, 6, 1, Bounds)),
    ?assertEqual(not_found, nit_hit:find_at(Tree, 8, 1, Bounds)).
...
status_bar_item_hit_in_vbox_with_spacer_test() ->
    ...
    ?assertEqual({status_bar_item, <<"H">>}, nit_hit:find_at(Tree, 2, 10, Bounds)).
...
unicode_status_bar_item_hit_test() ->
    ...
    Bounds = default_bounds(),
    ?assertEqual({status_bar_item, "↑/↓"}, nit_hit:find_at(Tree, 2, 1, Bounds)),
    ?assertEqual({status_bar_item, "↑/↓"}, nit_hit:find_at(Tree, 12, 1, Bounds)).
