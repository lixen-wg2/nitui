%%%-------------------------------------------------------------------
%%% @doc Unit tests for nit_focus module.
%%%
...
next_focus_empty_list_test() ->
    ?assertEqual(undefined, nit_focus:next_focus([], foo)).

next_focus_undefined_current_test() ->
    ?assertEqual(a, nit_focus:next_focus([a, b, c], undefined)).

next_focus_first_to_second_test() ->
    ?assertEqual(b, nit_focus:next_focus([a, b, c], a)).

next_focus_middle_to_next_test() ->
    ?assertEqual(c, nit_focus:next_focus([a, b, c], b)).

next_focus_wrap_around_test() ->
    ?assertEqual(a, nit_focus:next_focus([a, b, c], c)).

next_focus_not_found_returns_first_test() ->
    ?assertEqual(a, nit_focus:next_focus([a, b, c], unknown)).
...
prev_focus_empty_list_test() ->
    ?assertEqual(undefined, nit_focus:prev_focus([], foo)).

prev_focus_undefined_current_test() ->
    ?assertEqual(c, nit_focus:prev_focus([a, b, c], undefined)).

prev_focus_last_to_middle_test() ->
    ?assertEqual(b, nit_focus:prev_focus([a, b, c], c)).

prev_focus_middle_to_first_test() ->
    ?assertEqual(a, nit_focus:prev_focus([a, b, c], b)).

prev_focus_wrap_around_test() ->
    ?assertEqual(c, nit_focus:prev_focus([a, b, c], a)).

prev_focus_not_found_returns_last_test() ->
    ?assertEqual(c, nit_focus:prev_focus([a, b, c], unknown)).
...
collect_containers_empty_vbox_test() ->
    Tree = #vbox{children = []},
    ?assertEqual([], nit_focus:collect_containers(Tree)).

collect_containers_focusable_box_test() ->
    Tree = #box{id = my_box, focusable = true, children = []},
    ?assertEqual([my_box], nit_focus:collect_containers(Tree)).
...
collect_containers_tree_in_vbox_test() ->
    ...
    ?assertEqual([tree1], nit_focus:collect_containers(Tree)).
...
collect_containers_nested_in_vbox_test() ->
    ...
    ?assertEqual([box1, table1], nit_focus:collect_containers(Tree)).
...
collect_containers_nested_in_hbox_test() ->
    ...
    ?assertEqual([tabs1, box1], nit_focus:collect_containers(Tree)).
...
collect_containers_deeply_nested_test() ->
    ...
    ?assertEqual([deep_box], nit_focus:collect_containers(Tree)).
...
collect_children_box_with_buttons_test() ->
    ...
    ?assertEqual([btn1, btn2], nit_focus:collect_children(Tree, my_box)).
...
collect_children_box_with_inputs_test() ->
    ...
    ?assertEqual([input1, input2], nit_focus:collect_children(Tree, my_box)).

collect_children_box_with_table_test() ->
    Tree = #box{id = my_box, focusable = true, children = [
        #table{id = table1, focusable = true}
    ]},
    ?assertEqual([table1], nit_focus:collect_children(Tree, my_box)).

collect_children_box_with_scroll_test() ->
    Tree = #box{id = my_box, focusable = true, children = [
        #scroll{id = scroll1, focusable = true}
    ]},
    ?assertEqual([scroll1], nit_focus:collect_children(Tree, my_box)).
...
collect_children_tabs_returns_tab_ids_test() ->
    ...
    ?assertEqual([tab1, tab2, tab3], nit_focus:collect_children(Tree, my_tabs)).

collect_children_unknown_container_test() ->
    Tree = #vbox{children = []},
    ?assertEqual([], nit_focus:collect_children(Tree, unknown_id)).
...
find_element_button_test() ->
    ...
    Result = nit_focus:find_element(Tree, my_button),
...
find_element_input_test() ->
    ...
    Result = nit_focus:find_element(Tree, my_input),
...
find_element_deeply_nested_test() ->
    ...
    Result = nit_focus:find_element(Tree, deep_input),
...
find_element_in_tabs_content_test() ->
    ...
    Result = nit_focus:find_element(Tree, tab_button),
    ?assertEqual(tab_button, Result#button.id).

find_element_tree_test() ->
    Tree = #tree{id = my_tree, nodes = []},
    Result = nit_focus:find_element(Tree, my_tree),
...
find_element_tree_in_vbox_test() ->
    ...
    Result = nit_focus:find_element(Tree, nested_tree),
...
find_container_button_in_tabs_content_test() ->
    ...
    ?assertEqual(top_tabs, nit_focus:find_container(Tree, tab_btn)).
...
find_container_skips_unrelated_tabs_branch_test() ->
    ...
    ?assertEqual(actions_box, nit_focus:find_container(Tree, save_btn)).
